...
include('../conexao.php');
    ...
    for ($i = 0; $i < $qtd_produtos; $i++) {      
        if(strlen($produto_id[$i]) == 0 || strlen($qtd[$i]) == 0) {
            $vazio = true;
        }

        $sql = "SELECT * FROM produtos WHERE id_produto='$produto_id[$i]'";
            $consulta = mysqli_query($mysqli, $sql);
        
        if (mysqli_num_rows($consulta) == 0){
            $produto_inexistente = true;
        }
    }
        ...
        if ($vazio) {
            ...
            echo "Algum produto selecionado não foi encontrado!";
